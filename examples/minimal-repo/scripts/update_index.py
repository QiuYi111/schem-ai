...
def collect_files(project_root, prefix: str, suffixes: tuple[str, ...]) -> list[str]:
...
def read_approved_parts(project_root) -> list[str]:
    approved_path = project_root / "sourcing" / "approved_parts.yaml"
    payload = read_data(approved_path)
    if not payload:
        return []

    if isinstance(payload.get("approved_parts"), list):
        values = payload["approved_parts"]
    elif isinstance(payload.get("parts"), list):
        values = payload["parts"]
    else:
        values = []
    ...
    for item in values:
        if isinstance(item, str):
            ...
            value = item.get("mpn") or item.get("part_number") or item.get("name")
            if value:
