...
4. Keep artifact content aligned with the schemas in `schemas/`.
5. Call scripts at the required gates.
6. Use review loops before advancing phases.
7. Keep the repository auditable through index updates and checkpoints.
...
5. `scripts/validate.py`
   Use after artifact updates and before any review approval or phase transition. This script enforces the JSON Schema contracts in `schemas/`.
6. `scripts/review.py`
