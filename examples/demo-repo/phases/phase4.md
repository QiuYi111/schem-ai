...
`design/interconnect.json` must preserve an explicit evidence trail with:
...
Those fields belong in the top-level `evidence` collection and should connect major connectivity decisions back to architecture and handbook artifacts.
...
6. `design/interconnect.json` satisfies `schemas/interconnect.schema.json`
...
7. The evidence trail is missing or too weak for renderer traceability
