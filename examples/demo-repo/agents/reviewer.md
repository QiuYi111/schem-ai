...
When a phase defines structured evidence fields, verify that `source`, `rationale`, `confidence`, and `unresolved_items` are present and materially useful rather than filler.
...
5. Whether evidence fields are specific enough for later phases to trust the selection
...
4. Whether handbook evidence fields are strong enough to support phase4 reuse
...
5. Whether the evidence trail in `design/interconnect.json` is sufficient for traceability
